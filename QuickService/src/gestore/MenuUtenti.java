...
public MenuUtenti (Utente ut)
{
	utente=ut;
		
	UtilGC gc  = new UtilGC();
	final JFrame finestra = new JFrame("- Menu Gestore Utenti - ");
	...
	finestra.setLayout(new GridLayout(9,1));
        ...
        final JTextArea text = new JTextArea("   FUNZIONI NON IMPLEMENTATE  ");
        
	final JButton btnInserisciUtente = new JButton("Inserisci Utente");
	final JButton btnModificaUtente = new JButton("Modifica Utente");
	final JButton btnCancellaUtente = new JButton("Cancella Utente");
	
	final JButton btnIndietro = new JButton("Indietro");
        final JButton btnEnd      = new JButton("Esci");

        spazio.add(text);
	pannello.add(btnInserisciUtente);
        pannello1.add(btnModificaUtente);
        pannello2.add(btnCancellaUtente);
	...
	ActionListener al = new ActionListener(){
		...
		public void actionPerformed(ActionEvent evento){
			...
			if (btn ==btnInserisciUtente){
//				new GestioneInserisciUtente(utente);
				finestra.dispose();
			}
                        
                        if (btn ==btnModificaUtente){
//				new GestioneModificaUtentea(utente);
				finestra.dispose();
			}
			
                        if (btn ==btnCancellaUtente){
//				new GestioneCancellaUtente(utente);
				finestra.dispose();
			}
	...
	btnInserisciUtente.addActionListener(al);
        btnModificaUtente.addActionListener(al);
        btnCancellaUtente.addActionListener(al);
