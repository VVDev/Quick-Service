...
public class GestioneClienteFrame{
	
	private ArrayList<Attivita> attivita;
	private JTable TableAttivita;
	private Utente ut;
	 public GestioneClienteFrame  (Utente u)
	{
	 ut=u;
         UtilGC gc = new UtilGC();
	 final JFrame frmFinestraGS = new JFrame("Albergo - Seleziona Attivita");  //  @jve:decl-index=0:visual-constraint="135,33"
		 ...
		 public void mouseClicked(MouseEvent evt) {
			...
			private void ModificaSalaButtonMouseClicked(MouseEvent evt)throws SQLException {
				...
				if (riga == -1 ){
					...
					SelezionaAttivita mod= new SelezionaAttivita(ut,s);
		...
		frmFinestraGS.getContentPane().add(btnPanelSpazio);
		{
			...
			btnIndietro.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
                                    MenuCliente menu = new MenuCliente(ut);
					frmFinestraGS.dispose();
	 ...
	 lblPosti.setPreferredSize(new java.awt.Dimension(100, 14));
	  ...
	  lblPrezzo.setPreferredSize(new java.awt.Dimension(40, 14));
         ...
         frmFinestraGS.setIconImage(Toolkit.getDefaultToolkit().getImage(gc.getImagePaths()+"utenti.gif"));
